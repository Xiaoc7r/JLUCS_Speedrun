...
// 获取指定位置的棋子类型
inline int getPieceAt(AtaxxBoard *board, int x, int y) {
    return (board->boardState[x] >> ((y + 1) << 1)) & 3;
}

// 判断是否是当前玩家的棋子
inline int isCurrentPlayerPiece(AtaxxBoard *board, int x, int y) {
    return ((board->boardState[x] >> ((y + 1) << 1)) & 3) == board->playerColor;
}

// 判断位置是否为空
inline int isEmptyPosition(AtaxxBoard *board, int x, int y) {
